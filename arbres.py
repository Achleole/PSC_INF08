...
def creer_arbre(n):
    nextSite = NextSite(memLen=50000)
    univ = Univers.Univers(nextSite, TAILLE_MEMOIRE=50000, mutation=0)
...
def dessiner(arbre):
    ...
    for j in m:
        plt.plot([m[j][0],m[j][0]],[m[j][1],m[j][2]])
        if j!=1:
            plt.plot([m[j//2][0],m[j][0]],[m[j][1],m[j][1]])
    plt.show()     
